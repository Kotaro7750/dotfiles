...
-- providers setting
-- ----------------------------
-- Dynamically set host program paths generated by Nix
local ok, providers = pcall(dofile, vim.fn.stdpath("config") .. "/providers.lua")
if ok and providers then
  if providers.python3_host_prog then
    vim.g.python3_host_prog = providers.python3_host_prog
  end
  if providers.node_env_bin then
    local cur = vim.env.PATH or ""
    if not string.find(cur, providers.node_env_bin, 1, true) then
      vim.env.PATH = providers.node_env_bin .. ":" .. cur
    end
  end
  if providers.node_host_prog then
    vim.g.node_host_prog = providers.node_host_prog
  end
end
